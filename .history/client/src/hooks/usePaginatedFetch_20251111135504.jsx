...
export default function usePaginatedFetch(endpoint, initialParams = {}, deps = [], options = {}) {
  ...
  // Build the new params that we want to apply immediately
  const newParams = { ...params, ...override };

  // Build the key for the new params and set it as prevKey so effect won't re-trigger an identical fetch
  const newKey = (() => {
    try {
      return JSON.stringify({ endpoint, params: newParams, deps });
    } catch (e) {
      return `${endpoint}:${String(newParams.page)}:${String(newParams.limit)}:${String(newParams.search)}`;
    }
  })();

  // Update state to reflect the new params (avoid unnecessary re-render if same)
  setParams(prev => {
    if (JSON.stringify(prev) === JSON.stringify(newParams)) return prev;
    return newParams;
  });

  // Set prevKey to newKey so the effect guard won't run a duplicate fetch
  prevKeyRef.current = newKey;

  // Call doFetch with override so the request uses the intended params immediately
  doFetch(override);
};
