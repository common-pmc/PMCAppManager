...
const UserDashboard = () => {
  ...
  const fetchFiles = useCallback (async () => {
    try {
      ...
      setUserInfo ({
        company: response.data[0]?.company?.name || 'N/A',
        department: response.data[0]?.department?.name || 'N/A',
      });
      setLoading (false);
    } catch (error) {
      setError (
  ...
  return (
      <Box sx={{p: 3}}>
        ...
        {userInfo && (
          <Box>
            <Typography variant='subtitle1'><b>Фирма:</b> {userInfo.company}</Typography>
            <Typography variant='subtitle1' sx={{mb: 3}}><b>Отдел:</b> {userInfo.department}</Typography>
          </Box>
        )}
