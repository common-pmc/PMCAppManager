...
const AdminUserDetails = () => {
  ...
  } = usePaginatedFetch(`/admin/user/${id}`, { page: 1, limit: 10, search: '' }, [id], {
    debounceMs: 300,
  });
  ...
  useEffect(() => {
    ...
    async function loadUserFallback() {
      if (user) return; // има го вече
      try {
        const res = await axiosInstance.get(`/admin/user/${id}`);
        if (!mounted) return;
        if (res.data?.user) setUser(res.data.user);
      } catch (err) {
        // само логваме — показваме грешка по-долу ако е нужно
  ...
  // действия: toggle active, delete, download
  const handleToggleActive = async () => {
    try {
      ...
      if (res.data?.isActive !== undefined) {
        ...
        // ако не върне - презареждаме (fetchNow ще обнови и extra/downloads)
  ...
  // пагинация handlers
  const handlePageChange = (p) => fetchNow({ page: p });
  const handleLimitChange = (l) => {
    setLimit(l);
    fetchNow({ limit: l, page: 1 });
  };

  // търсене (enter -> незабавно)
  ...
  // комбинирано loading / error състояние
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
        <Typography variant="h6">Файлове, които потребителят е изтеглил</Typography>
        <TextField
          size="small"
          sx={{ width: 320 }}
          placeholder="Търси по име на файл и натисни Enter..."
          value={params.search || ''}
