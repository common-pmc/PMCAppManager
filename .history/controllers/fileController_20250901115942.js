...
exports.uploadFile = async (req, res) => {
  try {
    ...
    const user = await User.findOne ({where: {companyId, departmentId}});

    if (!user) {
      return res
        .status (400)
        .json ({message: 'Няма потребител за избраната фирма/отдел.'});
    }
          ...
          include: [
            {
              model: User,
              as: 'owner',
              attributes: ['companyId', 'departmentId', 'email'],
            },
          ],
