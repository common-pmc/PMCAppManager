...
exports.getFiles = async (req, res) => {
  try {
    ...
    // Ако не е админ → да вижда само файлове качени от него
    if (!user.isAdmin) {
    ...
    const formattedFiles = files.map(file => ({
      ...
      company: file.owner?.Company
        ? { id: file.owner.Company.id, companyName: file.owner.Company.companyName }
        : null,
      department: file.owner?.Department
        ? { id: file.owner.Department.id, departmentName: file.owner.Department.departmentName }
        : null,
